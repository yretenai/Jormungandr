	...
	public static unsafe void Decompress(CompressionType type, Memory<byte> compressed, Memory<byte> decompressed) {
		switch (type) {
			...
			case CompressionType.LZMA: {
				...
				using var inStream = new UnmanagedMemoryStream((byte*) inPin.Pointer, compressed.Length);
				using var outPin = decompressed.Pin();
				using var outStream = new UnmanagedMemoryStream((byte*) outPin.Pointer, decompressed.Length);
				var array = ArrayPool<byte>.Shared.Rent(5);
				try {
					var properties = array[..5];
					var coder = new Decoder();
					inStream.ReadExactly(properties);
					coder.SetDecoderProperties(properties);
					coder.Code(inStream, outStream, compressed.Length - 5, decompressed.Length, null);
				} finally {
